...
ArgoCD runs inside the ephemeral hcloud lab cluster and fetches the configured remote HTTPS repo.
If this is a private repo, provide credentials with ARGOCD_GITHUB_TOKEN,
...
wait_for_exists app-of-apps-hcloud-lab
wait_for_app app-of-apps-hcloud-lab Healthy
wait_for_exists platform-hcloud-lab
wait_for_app platform-hcloud-lab Healthy
wait_for_exists demo-api-hcloud-lab
wait_for_app demo-api-hcloud-lab Healthy
...
cat <<EOF
Hcloud ArgoCD reconciliation is synced and healthy on ${context}.
Verified ordering intent: app-of-apps-hcloud-lab creates platform-hcloud-lab before demo-api-hcloud-lab via sync waves.
EOF
