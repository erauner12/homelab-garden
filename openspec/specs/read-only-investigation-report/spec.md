## Purpose
Provide a deterministic, read-only investigation workflow that gathers Kubernetes, GitOps, rollout, workload, health, event, and log context for `demo-api` without mutating cluster state.
